...
class Ciclos{
    ...
    public function CicloProyectos($variable){
        while ($proyecto =$variable->fetch(PDO::FETCH_ASSOC)) {
            ...
            if($rol != 3){
                echo "<input class='aban' type='button' name='abandonar' value='Abandonar Proyecto' onclick='abandonarProyecto(".$proyecto['idproyecto'].")'>";
    ...
    public function CicloSprint($variable){
        while ($sprint =$variable->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo '<img  id="imcar" src = "../img/fondo.svg" width = "64px">';
            echo '<p>Fecha de inicio: '.$sprint['finicio'].'</p>';
            echo '<p>Descripción:'.$sprint['descripcion'].'</p>';
            echo '</a>';
            echo "<a href=\"update-sprint.php?idsprint=$sprint[idsprint]\"><h5>Modificar sprint</h5></a>";
            echo "<input class='but' type='button' name='eliminar' value='Eliminar Sprint' onclick='deleteSprint(".$sprint['idsprint'].")'>";
            echo "<br><input class='but' type='button' name='finalizar' value='Finalizar Sprint' onclick='finalizarSprint(".$sprint['idsprint'].")'>";
            echo '</div>';
            echo '</div>';
        }
    }
